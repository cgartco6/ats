#!/usr/bin/env python3
"""
Sovereign Enterprise System - Zero-Fail Meta-Builder
File: builder.py
Purpose: Reads a structural system JSON blueprint and systematically compiles
         ...
         Includes automated exponential backoff retries to handle 503/429 errors.
"""
...
def clean_model_response(raw_text: str) -> str:
    """
    Strips markdown code fences (```python ... ```) if returned by the model
...
def run_meta_builder():
    # 1. Access your API key directly inside the local environment
    api_key = "AIzaSy..."  # <--- PASTE YOUR COPIED GEMINI API KEY ENTIRELY HERE
    
    if not api_key or api_key == "AIzaSy...":
        print("[-] Error: Please paste your active Gemini API key directly onto line 45 of builder.py.")
        sys.exit(1)
    ...
    # Fallback to alternative blueprint names if present in workspace
    if not os.path.exists(blueprint_file):
        if os.path.exists("apex_blueprint.json"):
            blueprint_file = "apex_blueprint.json"
        elif os.path.exists("print.json"):
            blueprint_file = "print.json"
        elif os.path.exists("career_blueprint.json"):
    ...
    system_instruction = (
        ...
        "2. 2026 ARCHITECTURE: Code must use native asynchronous execution blocks (async/await), "
    ...
    for module_idx, module in enumerate(modules, start=1):
        ...
        for file_info in files:
            ...
            # Persistent retry loop variables
            success = False
            delay = 10  # Start with a base 10-second wait on failure
            ...
            while not success:
                try:
                    ...
                    # Stable cooldown spacing between successful file creations
                    time.sleep(6)
                    
                except APIError as api_err:
                    print(f"        [!] Server Congestion ({api_err.code}). Retrying in {delay} seconds...")
                    time.sleep(delay)
                    delay = min(delay * 2, 60)  # Exponentially escalate backoff time up to 60s max
                    
                except Exception as error:
                    print(f"        [X] Unexpected structural anomaly: {error}")
                    print("        [*] Retrying generation step to preserve run integrity...")
                    time.sleep(5)
